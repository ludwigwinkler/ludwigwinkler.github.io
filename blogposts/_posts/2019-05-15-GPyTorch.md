...
[Gaussian Processes](https://ludwigwinkler.github.io/blog/Gaussian-Processes/) are a class of popular and powerful probabilistic models which can be derived straight from a Normal distribution.
...
These kernel matrices over the entire training data set and their numerous use in matrix matrix products, log determinants and inverses result in poor scalability to large data sets.
...
Considerable focus will be on conjugate gradient descent which allows the efficient optimization of quadratic optimization problems through the use of line search and conjugate search directions.
