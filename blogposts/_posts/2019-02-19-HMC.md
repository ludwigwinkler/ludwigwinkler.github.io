...
The $\Delta x$ is required since we are approximating the function $f( x \| \\mu, \\sigma)$ with little columns where the exponential term tells us the height and $\Delta x$ the width of the column (look at the animations at Wikipedias entry for Riemann integral).
...
Remember that the bulk of the integral $\smash{\int_{-\infty}^{\infty} f(x|\mu, \sigma) dx}$ is evaluated around $\mu$.
...
Obviously we could restrict the uniform distribution $\mathcal{U}(x_{min}, x_{max})$ to be closer to the value of $\mu$ but this could only be efficiently done if we were to have specific prior information about the distribution.
...
The main takeaway point is that one has to pay close attention to what the sampler is doing and how sampling algorithms are designed to make efficient use of computational resources.
...
The proposal sample $x'_t$ can be rejected and in that case a new proposal sample $x'_t$ has to be proposed.
...
Let's say you have physical system $\mathcal{H}(x, p, t)$ in which two properties describe everything you know.
...
The two differential equations above describe how the energy of the system is allocated to either the momentum $p(t)$ or the position $x(t)$ as time progresses.
...
(That's actually something we want to prevent which led to the development of the No-U-Turn-Sampler (NUTS)).
...
For the broader Bayesian inference context in which MCMC sits — and the variational alternative — see [Intro to Bayesian Inference](https://ludwigwinkler.github.io/blog/BayesianInference/). For a staged MCMC scheme that bridges easy and hard target distributions, see [Annealed Importance Sampling](https://ludwigwinkler.github.io/blog/AISSMC/), and for diagnosing how much information your correlated chain actually carries, see [The Weight of Sampling](https://ludwigwinkler.github.io/blog/ESSSMC/).
