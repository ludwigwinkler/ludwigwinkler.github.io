...
We will assume a [stochastic differential equation](https://ludwigwinkler.github.io/blog/SDE/) the first two orders of which can be estimated with
$$
\begin{align}
	M^{(n)} (x'_t) = \int (x_{t+\tau} - x'_t)^n  p(x_{t + \tau} | x'_t) dx_{t+\tau}
\end{align}
$$
such that the dynamics are described by the [Ito drift-diffusion process](https://ludwigwinkler.github.io/blog/ItosLemma/)
$$
\begin{align}
...
The Chapman-Kolmogorov equation for the forward Kramers-Moyal expansion can be rewritten with the help of an auxiliary variable as
$$
\begin{align}
...
If the function $f(x)$ has compact support, meaning that $\{f(x)=0 | x = \pm \infty \}$, the evaluation of the derivative-free component in integration by parts vanishes.
...
\end{align}
$$
which more or less moves all the derivatives from the Dirac delta function over to $f(x)$ which is often called the 'test function' when inside an integral over the full domain.
...
Finally we can note that we could cut off the Taylor expansion after the second order and realize that Taylor expansion is equivalent to the time derivative in the limit of time, i.e. $$\lim_{\tau \rightarrow 0}$$ and we can proclaim that
$$
\begin{align}
...
The Kolmogorov backward equation (KBE) can be derived in the same way while paying attention to the derivatives. The KBE is the key ingredient for deriving the [reverse-time SDE (Anderson 1982)](https://ludwigwinkler.github.io/blog/ReverseTimeAnderson/), [Doob's h-transform](https://ludwigwinkler.github.io/blog/Doob/), and the [Feynman-Kac formula](https://ludwigwinkler.github.io/blog/FeynmanKac/).

In the Kolmogorov forward equation the differential operators $$M^{(n)}$$ were defined for stochastic variables following the natural arrow of time.
