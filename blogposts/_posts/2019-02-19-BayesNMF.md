...
This post assumes familiarity with the deterministic version covered in [Non-Negative Matrix Factorization](https://ludwigwinkler.github.io/blog/NMF/).
...
In order to allow PyTorch (shout out to the devs!) to backpropagate to the variational parameters $\mu, sigma$ we have to reparameterize every entry in the parameters.
...
After computing the cost function we can let PyTorch do its automatic differentiation magic and we optimize the whole thing with an optimizer such as Adam.
        ...
        class BayesNMF(torch.nn.Module):
                ...
                def forward1(self):
                        '''
                        Sample multiple parameters in parallel
                ...
                def full_matrix(self):
                        """
                        Compute a single full matrix using the resultant biases, P and Q
                        """
...
Upon inspection of the mean $\mu_{\hat{R}}$ and $\sigma_{\hat{R}}$ we can see that the variance in the fourth row is markedly increased:
