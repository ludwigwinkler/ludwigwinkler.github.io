...
I encountered Girsanov's theorem repeatedly as I was reading stochastic processes papers and talking to people from that community.

A quick look at [Wikipedia](https://en.wikipedia.org/wiki/Girsanov_theorem) returned something that was absolutely unintelligible to me (without a proper higher math education).

Thus began a chase down the rabbit hole for alternative sampling in stochastic processes. :-)

A natural follow-up is [Doob's h-transform](https://ludwigwinkler.github.io/blog/Doob/), which uses similar reweighting ideas to condition a stochastic process on a future event.

### Importance Sampling

The first thing that I noticed was that the term $$\frac{dQ}{dP}$$ occurred in a lot of text books that I found on Google.
...
Let's assume we have two [stochastic processes](https://ludwigwinkler.github.io/blog/SDE/) $\{X^p_t\}$ and $\{X^q_t\}$,
<div style="overflow-x: auto;">
$$
\begin{align}
  dX^q_t &= a_t dt + dW_t \\
...
Next, we would like to compute the expected value over some function of the realizations of the stochastic process $f(\{X^q_t\})$,
<div style="overflow-x: auto;">
$$
\begin{align}
...
But fortunately for us, we can apply the intuition that we gained from the importance sampling approach and apply it here as well.
...
This is due to the fact that in a Wiener process, every movement is independently and identically distributed (iid).
...
$$
\begin{align}
\lim_{\Delta t \rightarrow 0^+} \sum_{t_k} (x_{t +\Delta t} - x_t) a_t &= \int a_t dX_t \quad \text{([Ito Integral](https://ludwigwinkler.github.io/blog/ItosLemma/))} \\
...
We draw a series of conclusions from the plot above:
- We sample two realizations of the stochastic process $X^p_{t,1}$ and $X^p_{t,2}$.
- Visually, we can see that whereas $X^p_{t,1}$ 'hit' the target measure $\mathbb{Q}$ quite well, $X^p_{t,2}$ did almost not at all.
...
- The contribution of $X^p_{t,2}$ to the expected value on the other hand has to be downscaled/graded significantly.
