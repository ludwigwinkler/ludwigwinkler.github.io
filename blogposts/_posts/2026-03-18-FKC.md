...
For notational simplicity, let us define $\tilde{p} = \int q_t \exp(\beta_t r(x)) dx$
<div style="overflow-x: auto;">
$$
\begin{align*}
...
So we expand the $\log q_t$ in the same way as we did above, and then multiply it with the $p_t$ in the integral, we get
<div style="overflow-x: auto;">
$$
\begin{align*}
\partial_t \log Z_t 
...
A straightforward option would be to evolve this very particular PDE backward in time form $T$ to an earlier $t$ and then read off the solution at $t=0$. This would give us $u(x,t)$.
