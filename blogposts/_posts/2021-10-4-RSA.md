...
But how did he obtain the knowledge to decrypt the encrypted text?

Certainly, it's very dangerous to simply send the decryption key, whatever form it may take, through the plain mail.
It could get intercepted and copied, rendering any further encrypted communication worthless, as the copied decryption key allows the interceptor to decrypt the secret message.
...
So another person who wants to send you encrypted messages can simply pick up a publicly available open lock, put the message inside, lock it and send you the message.
...
If you were to reply, you would simply pick up a lock that the other person has publicly on offer, lock your message up and send him or her your message.

This concept is nice in theory, but how would it work technically and provably mathematically?
...
### Euler's Theorem: The Bedrock of RSA
...
The notation $[ \ldots ] \mod n$ means that we apply the modulo equally to both sides. The notation $\ldots = \ldots (\mod n)$ is more common but I like my parentheses to clearly delineate what operator we apply to what.
...
For this we will use the previously stated property of the totient that if two numbers are coprime to each other, we can simply factorize the totient into the multiplicative components.
...
The receiver can then exponentiate the 'decrypted' hash again with the public key $e$ and take the modulo of it and compare it to his or her own hash of the sent message.
