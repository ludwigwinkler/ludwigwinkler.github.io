...
It is certainly nice to know that if we run those $n=100$ trials a million times, we will eventually converge to $k=60$ successful runs per $n=100$ trials with a success probability of $p=0.6$.
...
The first time we draw, we have 100 options, but the second time we draw we only have 99 options left and the third time we draw we only have 98 left and so on.
...
The definition above unfortunately only tells us the number of events occurring over the entire interval $(t_0, t_1]$.
What if we wanted to know the probability of an event happening at a singular moment $t$?
...
On the flip side, this means that the probability of an event occurring is monotonically increasing as time progresses:
$$
\begin{align}
...
Since $\mathbb{P}[N(t_0, t_1]=1, \lambda(s)]$ is a cumulative probability distribution as it approaches 1 in infinity, we can simply derive it with respect to $t_1$ to obtain the probability density function:
$$
\begin{align}
...
For an extension that uses Poisson rates to define continuous-time stochastic processes over discrete states, see [Continuous Time Markov Chains in Jax](https://ludwigwinkler.github.io/blog/ContTimeMarkovChain/) and the [Ehrenfest process](https://ludwigwinkler.github.io/blog/TheEhrenfestProcess/) post.