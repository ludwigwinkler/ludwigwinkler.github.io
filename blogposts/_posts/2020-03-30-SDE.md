...
Below is an image juxtaposing what we refer to as non-differential equations and differential equations with respect to time:
...
The grey lines in the right plot model the changing temperatures over time of three coffee mugs with different temperatures.
...
But before we can proceed to stochastic differential equations, we have to talk about stochasticity over time.
...
Secondly we will choose the "amount of time $dt$" to actually be $\sqrt{dt}$, the reason of which will be clear in an instant.

Thirdly, we want the particle to start at zero, so $x_0 = 0$.
...
Basically all the paths stay around zero where they start and they spread out according to our analytically computed variance of $\mathbb{V}[x_t] = t$ over time.
...
But [Ito's lemma](https://ludwigwinkler.github.io/blog/ItosLemma/), [Ornstein-Uhlenbeck processes and Geometric Brownian Motion](https://ludwigwinkler.github.io/blog/SolvingSDEs/) are topics for another time ...

For the distributional view of the same dynamics — i.e. how the entire probability density $p(x,t)$ evolves in time rather than a single trajectory — see the [Fokker-Planck equation](https://ludwigwinkler.github.io/blog/FokkerPlanck/) and the [Kramers-Moyal expansion](https://ludwigwinkler.github.io/blog/Kramers/). For the time-reversed counterpart used in generative diffusion models, see [Simple Reverse-Time SDEs](https://ludwigwinkler.github.io/blog/SimpleReverseSDE/).