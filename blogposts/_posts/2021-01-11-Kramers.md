...
Since for any subtraction $$x'_{t} - x_{t+\tau}$$ and function $$f(x'_{t} - x_{t+\tau})$$ we can employ the relation $$\partial_{x'_t} \ f(x'_{t} - x_{t+\tau}) = - \partial_{x_{t+\tau}} \ f(x'_{t} - x_{t+\tau})$$ via the change of variables, we switch the derivative to 
$$
\begin{align}
	\sum_{n=0}^\infty \frac{1}{n!} ( y_{t+\tau} - x'_t)^n \ \partial_{x'_{t}}^{n} \ \delta(x'_t - x_{t+\tau}) = & \sum_{n=0}^\infty \frac{1}{n!} ( y_{t+\tau} - x'_t)^n \ (- \partial_{x_{t+\tau}})^{n} \ \delta(x'_t - x_{t+\tau})
\end{align}
$$

In essence, we're computing the _forward_ difference over all possible $x_{t+\tau}$ for a known, and thus fixed, $x_t$.
...
$$
\begin{align}
	&\int_Y \sum_{n=0}^\infty \frac{1}{n!} ( y_{t+\tau} - x'_t)^n \ (- \partial_{x_{t+\tau}})^{n} \ \delta(x'_t - x_{t+\tau}) p(y_{t+\tau} | x'_t) dy_{t+\tau} \\
	= & \Big( 1 + \sum_{n=1}^\infty \frac{1}{n!} (- \partial_{x_{t+\tau}})^{n} \underbrace{\int_Y ( y_{t+\tau} - x'_{t})^n \ p(y_{t+\tau} | x'_t) \ dy_{t+\tau}}_{M^{(n)}(x'_t)} \Big) \ \delta(x'_t - x_{t+\tau}) \\
	= & \Big( 1 + \sum_{n=1}^\infty \frac{1}{n!} (- \partial_{x_{t+\tau}})^{n} M^{(n)}(x'_t) \Big) \ \delta(x'_t - x_{t+\tau})
\end{align}
$$
...
\begin{align}
	...
	= & \int_{X'} \Big( 1 + \sum_{n=1}^\infty \frac{1}{n!} (- \partial_{x_{t+\tau}})^{n} M^{(n)}(x'_t) \Big) \ \delta(x'_t - x_{t+\tau}) \ p(x'_t) \ dx'_t \\
	= & p(x_t) + \underbrace{\sum_{n=1}^\infty \frac{1}{n!} (- \partial_{x_{t+\tau}})^{n} \left[ M^{(n)}(x_t) \ p(x_t) \right]}_{\lim_{\tau \rightarrow 0} : \partial_t p(x_t) \tau}
\end{align}
$$
...
$$
\begin{align}
	p(x''_{t+\tau} | x'_t) = \int \delta(y_{t+\tau} - x''_{t+\tau}) p(y_{t+\tau} | x'_{t}) dy_{t+\tau}
\end{align} 
$$
...
\begin{align}
	...
	= & \delta( y_{t+\tau} - x'_t + x'_t - x''_{t+\tau}) \\
