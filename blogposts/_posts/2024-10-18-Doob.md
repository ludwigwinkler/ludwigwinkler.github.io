...
Previously, we saw how [Girsanov's theorem](https://ludwigwinkler.github.io/blog/Girsanov/) is little else than importance sampling for stochastic processes.

This time I stumbled over _Doob's h-transform_.
On first reading it is an equally strange equation which seemed hard to understand when simply googling for it.
But peeling away the layers like an onion and almost crying an equal amount along the way, there were some simple and intuitive things to be found.
...
Obviously this only holds in the expected sense and sure enough there can be massive spikes when considering extreme paths.
...
For a derivation of the Kolmogorov backward equation via the Kramers-Moyal expansion, see [Fokker, Planck & Kolmogorov Revisited](https://ludwigwinkler.github.io/blog/Kramers/).
...
So, it's time to use [Ito's lemma](https://ludwigwinkler.github.io/blog/ItosLemma/) once again!
...
Whereas the FPE/KFE models the interaction between the drift, diffusion and the resulting marginal probability distribution, the KBE only evaluates a function which never interferes with the dynamics.
...
The three terms above say the following: The expected, infinitesimal change of a function, the input of which is driven by a SDE, can be written as partial differential equation.
...
With that proven, we can now define an infinitesimal generator under the transition kernel $\tilde{p}$.
This is a kind of expected finite difference between a state $x_t$ and a next state $x_{t+\Delta t}$ where we take the expected value over the slightly modified transition kernel $\tilde{p}$.
We have
<div style="overflow-x: auto;">
$$
\begin{align}
...
In order to alleviate the notational burden, we will denote $f(x_t, t)$ without its arguments and abbreviate $p(x_T, T | x_t, t)$ by $h$.
<div style="overflow-x: auto;">
$$
\begin{align}
...
So far, we've only differentiated once the functions $f: \mathbb{R}^N \rightarrow \mathbb{R}$ and $p: \mathbb{R}^N \rightarrow \mathbb{R}$ with $x \in \mathbb{R}^N$.
...
Remember how the Kolmogorov backward equation was derived for a standard Ito SDE $dx_t = \mu(x_t, t) dt + \sigma(x_t, t) dW_t$?
...
Interestingly, this looks extremely similar to [reverse-time diffusion](https://ludwigwinkler.github.io/blog/SimpleReverseSDE/) as it's used in diffusion based generative modelling in machine learning.